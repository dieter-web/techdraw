...
renderSVG (Rect (x,y) (w,h) stroke fill) =
  ...
  ++ renderMaybeStroke stroke ++ " " ++ renderFill fill ++ " />\n"
...
renderSVG (Polyline pts stroke) =
  ...
  ++ "\" " ++ renderMaybeStroke stroke ++ " fill=\"none\" />\n"
...
renderSVG (Path cmds stroke fill) =
  "<path d=\"" ++ unwords (map renderPathCommand cmds) ++ "\" "
  ++ renderMaybeStroke stroke ++ " " ++ renderFill fill ++ " />\n"

renderSVG (Text (x,y) anchor content) =
  "<text text-anchor=\"" ++ show anchor ++ "\" x=\"" ++ show x ++ "\" y=\"" ++ show y ++ "\">"
  ++ content ++
  "</text>\n"
